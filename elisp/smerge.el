...
(defun smerge-ediff-or-copy (arg)
  ...
  (let* ((extent (extent-at (point)))
    ...
    (setq smerge-extent extent)
    (cond 
     ;; 1 RET means choose file from 1st dir
     ((and (member type '(3 2)) (eql arg 1))
      (setq smerge-file (smerge-file extent))
      ;; copy into 2
      (smerge-copy 2 nil))
     ;; 2 RET mean schoose file from 2nd dir
     ((and (member type '(3 2)) (eql arg 2))
      (setq smerge-file (smerge-file extent))
      ;; copy into 1
      (smerge-copy 1 nil))
     ((or (eql type 1) (eql type 2))
      (setq smerge-file (smerge-file extent))
      (smerge-copy type t))
     ((eql type 3) (smerge-ediff extent))
     (t (beep)))))
