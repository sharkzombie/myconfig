...
(defvar cff-use-other-window nil
  "When set or bound to t, cff will use `find-file-other-window' instead of `find-file'")


(defun cff--find-file (name)
  "Wrapper that calls either `find-file' or `find-file-other-window' depending on `cff-use-other-window' variable"
  (if cff-use-other-window (find-file-other-window name)
    (find-file name)))
...
;;;###autoload
(defun cff-find-file-other-window (prefix) 
  "Find the appropriate header, source or interface file for the current file in the other-window.
...
for multiple possible file variants; otherwise use the cached file name."
  (interactive "P")
  (let ((cff-use-other-window t)) 
    (cff-find-other-file prefix)))
...
for multiple possible file variants; otherwise use the cached file name."
  ...
  (let* ((fname (expand-file-name (buffer-file-name)))     ; full file name
    ...
    (if (and cff-mutiple-values-cache
        ...
        (cff--find-file (gethash fname cff-mutiple-values-cache))
