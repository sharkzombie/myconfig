" -*- vim -*-
" @(#) $Id: savevers.vim,v 1.3 2005/01/17 17:32:51 max Exp $
"
...
"
" $Log: savevers.vim,v $
" Revision 1.3  2005/01/17 17:32:51  max
" Remove the triggers thing, since it screws up all the time
" On Win32 make the .backup directory hidden
"
" Revision 1.2  2004/05/14 17:46:04  max
" Added auto-make of versioned backup directories
"
" Revision 1.1  2004/05/13 23:08:47  max
" Added the versioned backups thing
"
...
function! s:next_versdir(base)
   while strlen(s:verspath_iterator)
      ...
      if filewritable(l:dir) != 2
         "call confirm('Directory ' . l:dir . ' is not writable', 'ok')
         if exists("g:savevers_dirs_make") && strlen(g:savevers_dirs_make)
            "call confirm('Making directory ' . l:dir . ' with command ' . SysMkdir(l:dir), 'ok')

            call system(SysMkdir(l:dir))
            call system(SysMkHid(l:dir))
         endif
         continue
      endif
      return l:dir
